...
def initializePlugin(obj):
    plugin = OpenMayaMPx.MFnPlugin(obj, 'Example_sub', '2.0', 'Any')
    try:
        plugin.registerNode(
            'ExampleSubDataNode',
            ExampleSubDataNode.kPluginNodeId,
            creator,
            initialize,
        )
    except Exception:
...
def uninitializePlugin(obj):
    ...
    try:
        plugin.deregisterNode(ExampleSubDataNode.kPluginNodeId)
    except Exception:
        raise RuntimeError('Failed to register node')


def creator():
    return OpenMayaMPx.asMPxPtr(ExampleSubDataNode())
...
class ExampleSubDataNode(OpenMayaMPx.MPxNode):
